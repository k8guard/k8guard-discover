...
func verifyRequiredAnnotations(annotations map[string]string, entity *lib.ViolatableEntity, entityType string, violation violations.ViolationType) {
	...
	if rules.IsNotIgnoredViolation(entity.Namespace, entityType, violations.REQUIRED_ANNOTATIONS_TYPE) {
		lib.Log.Info("   not ignored")
		if found, source, _ := rules.IsValuesMatchesRequiredRule(entity.Namespace, entityType, annotations, lib.Cfg.RequiredAnnotations); !found {
...
func verifyRequiredLabels(labels map[string]string, entity *lib.ViolatableEntity, entityType string, violation violations.ViolationType) {
	if rules.IsNotIgnoredViolation(entity.Namespace, entityType, violations.REQUIRED_LABELS_TYPE) {
		if found, source, _ := rules.IsValuesMatchesRequiredRule(entity.Namespace, entityType, labels, lib.Cfg.RequiredLabels); !found {
			entity.Violations = append(entity.Violations, violations.Violation{Source: source, Type: violation})
		}
	}
}
